...
#include "../third_party/symbolize/symbolize.hh"
#endif
...
void DemangleSymbols(std::string* text) {
  ...
  while (search_from < text->size()) {
    ...
    int status = 0;
    std::unique_ptr<char> demangled_symbol(abi::__cxa_demangle(mangled_symbol.c_str(), NULL, 0, &status));
//    scoped_ptr_malloc<char> demangled_symbol(abi::__cxa_demangle(mangled_symbol.c_str(), NULL, 0, &status));
...
bool GetBacktraceStrings(void *const *trace, int size,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (google::Symbolize(static_cast<char *>(trace[i]) - 1, symbol, sizeof(symbol))) {
      // Don't call DemangleSymbols() here as the symbol is demangled by
      // google::Symbolize().
      trace_strings->push_back(chromium::StringPrintf("%s [%p]", symbol, trace[i]));
      symbolized = true;
    } else {
      trace_strings->push_back(chromium::StringPrintf("%p", trace[i]));
    }
  }
#else
  std::unique_ptr<char*> trace_symbols(backtrace_symbols(trace, size));
